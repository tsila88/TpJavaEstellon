...
public class ProductController {
	...
	@Autowired
	ProductService ps; // nous aurons donc besoin d'en faire un mock dans le test
	
	@Autowired
	CategoryService cs; // nous aurons donc besoin d'en faire un mock dans le test
...
//			model.put("products", ps.getCategoryProducts(cs.getCategory(request.getParameter("category")).get()));
			Optional<Category> opCat = cs.getCategory(request.getParameter("category"));
			if(opCat.isPresent()) model.put("products", ps.getCategoryProducts(opCat.get()));
		}
		return "homepage";
	}
}
